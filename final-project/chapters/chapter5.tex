...
% \begin{definition}\label{def:point_spectrum}\label{def:eigenvalues}
...
% \begin{example} 
%     In this example we're going to show a case of a linear operator in an infinite dimensional space for which $\sigma_p(T) \neq \sigma(T)$. Define $X = \ell^2(\N \to \C)$ as the set of sequences $x = (x_1,x_2,\ldots)$ that satisfy
...
% In this section, based on Chapter VII of~\cite{dunford1988linear}, we're going to discuss the case where $X$ is a finite dimensional complex Banach space and $T: X \to X$ is a linear operator. According to the last remark, $\sigma_p(T) = \sigma(T)$, so we are allowed to call $\sigma(T)$ the eigenvalues of $T$. For $\lambda \in \sigma(T)$ there exists multiple solutions $x \in X$, $x\neq 0$ for the equation $(\lambda I -T)x = 0$. These solutions are called the \textit{eigenvectors} and the set that contains all the solutions for the previous equation: $N_\lambda(T) := \ker(\lambda I - T)$ is called the \textit{geometric eigenspace} in $\lambda$. 
...
% In general, we can prove that $X$ is decomposed by the direct sum of the algebraic eigenspaces of any linear operator $T: X \to X$. But, before that, we want to show a important theorem that states that for any polynomial $P : \C \to \C$ and a linear operator $T$, the operator given by $P(T)$ is identically zero if and only if for every $\lambda \in \sigma(T)$, the multiplicity of $\lambda$ as a zero of $P$ coincides with the index $v(\lambda)$.

% \begin{theorem}
%     For a complex polynomial $P$ and a linear operator $T: X \to X$ the following conditions are equivalent:
%     \begin{itemize}
%         \item[(a)] $P(T) = 0$
...
% \begin{proof}
%     \textbf{Preliminaries:} Since we're assuming that $X$ is finitely dimensional, let $n = \dim X$ and let $\{x_1,\ldots, x_n\}$ be a basis of $X$. Then, for every $k = 1,\ldots, n$, the set $\{x_k, T x_k, \ldots, T^n x_k\}$ with $n+1$ elements is linearly dependent, so there exists a non-zero and non-constant polynomial $S_k$ such that $S_k(T)x_k = 0$. Define the linear operator $R = S_1 \cdot S_2 \cdots S_n$ which satisfies $R(T) x_k$ for every $k = 1,\ldots, n$, and thus, since every $x \in X$ can be written as a linear combination of basis elements $x = a_1 x_1 + \cdots + a_n x_n$. It follows that for any linear operator $T$ there always exists a non-zero polynomial $R:\C \to \C$ such that $R(T) x = 0$ for every $x \in X$.
...
% \begin{corollary}
%     For a finite dimensional space $X$, the spectrum of a linear operator $T: X \to X$ is non-empty and finite.
% \end{corollary}
% $ $\hfill $\square$
...
% \begin{theorem} For every function $f\in \F(T)$,
